			...
			<div class="m_datatable m-datatable m-datatable--default m-datatable--loaded" style="">
                        ...
                        <table  id=""  class="table table-hover table-striped">
                            ...
                            <tr class="netTr">
								...
								<th><?php echo $this->lang->line('General_Time');?></th>
								<th><?php echo $this->lang->line('Subject');?></th>
								<th><?php echo $this->lang->line('General_Type');?></th>
							...
							<tr class="<?php if(getMissionCount( $appoinment['id'],"general_mission") > 0) echo 'issubm'; ?>" >
							<td><?= $count++ ?></td>
						...
						<td><?= general_type($appoinment['general_type']) ?></td>
...
<td><a href="" class="num_tab" style="background-color: green;"><?php echo	getMissionCount( $appoinment['id'],"consultation_mission"); ?></a> </td>
							...
							<td class="action">
							<a href=<?= base_url("front/view_general_appoinment/{$appoinment['id']}") ?>  title="<?php echo $this->lang->line('View');?>" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" target="_blnak">  <i class="fa fa-eye"></i> </a><br>
...
$appoinments =  $this->db->select("general_mission.*,c_case.id as case_id,c_case.client_name,c_case.case_number,c_case.opponent_full_name,c_case.case_title,c_case.court_name,c_case.judge_name")
->join('c_case', "c_case.id = general_mission.case_id", 'left')
->where("(c_case.customers_id = $id  AND status=0)", NULL, FALSE)->where("general_mission.sub_mission_id",$appoinment["id"])
->get('general_mission')
->result_array(); 
?>
