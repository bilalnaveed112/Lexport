<?php include __DIR__ . "/../header.php"; ?>
                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">
                               <?php if($data){ ?>
				<?php echo $this->lang->line('Edit_Visiting');?>
			<?php	} else { ?>
			<?php //sub
				if($this->uri->segment(5)){
				echo "Add Visting Sub Mission For #".getMissionNumber($this->uri->segment(5),"visiting_mission");
				} else {
				    echo $this->lang->line('ADD_VISITING'); 
				} 
				//sub
				?> 
 ...
 //sub
		...
		?>
			...
			<label for="session_number" class=" form-control-label"><?php echo $this->lang->line('Visiting_Number');?></label>
		<?php if($data)
			{
			...
			{
			    if($sub_mission_id){
		        $sct = str_pad(getMissionCount($sub_mission_id,"visiting_mission") + 1, 2, 0, STR_PAD_LEFT);
			    	$session_number=set_value('session_number',getMissionNumber($this->uri->segment(5),'visiting_mission')."/".$sct);
				} else {
			    	
				$session_number=set_value('session_number', mission_rand($case_id,'VN'));
				}
			} ?>
		...
		<div class="form-group col-sm-4">
			...
			<div class="input-group date">
		 	<?= form_input(['name'=>'session_time','id'=>'session_time','required'=>'required','autocomplete'=>'off','class'=>'deedate form-control','value'=>$value]);?>
			<div class="input-group-append">
		...
		<div class="form-group col-sm-4">
			<label for="session_end_date" class=" form-control-label"><?php echo $this->lang->line('Visiting_End_Date');?>*</label>
			<?php if($data)
			{
		...
		<div class="form-group col-sm-4">
			<label for="session_time" class=" form-control-label"><?php echo $this->lang->line('Visiting_End_Time');?>*</label>
			<?php if($data)
			{
...
<?php echo $js["judicial_circuit"]?></option><?php } ?></select>
		...
		<div class="form-group col-sm-6">
			...
			{
				$value=set_value('visiting_place');
			} ?>
			<?= form_input(['name'=>'visiting_place','class'=>'form-control','value'=>$value]);?>
			<div class="form-error"><?= form_error('visiting_place'); ?></div>
		</div>
		
		<div class="form-group col-sm-4">
			<label for="who_visit" class="form-control-label"><?php echo $this->lang->line('Who_visit_to_who');?></label>
			<?php if($data)
			{
				$value=$data->who_visit;
			}
			else
			{
				$value=set_value('who_visit',$value);
			} 
			 $tt  =  mb_substr($value, 0, 1);
			?>
			<select class="form-control" id="who_visit" name="who_visit" >
			<option value=""><?php echo $this->lang->line('Select_Who_visit_to_who');?></option>  
			<option value="Employee Visiting to client" <?php if($tt=="E")  echo "selected=selected"; ?> ><?php echo $this->lang->line('Employee_Visiting_to_client');?> </option>  
			<option value="Client Visiting to employee" <?php if($tt=="C")  echo "selected=selected"; ?> ><?php echo $this->lang->line('Client_Visiting_to_employee');?> </option>  
			</select>
			<div class="form-error"><?= form_error('who_visit'); ?></div>
		</div>
		...
		<div class="form-group col-sm-4">		
			...
			<?php if($data)
			{
				$value=$data->visitor_employee;
			}
			else
			{
				$value=set_value('visitor_employee');
			}  ?>
	
		<select class="form-control" id="employee_id" name="visitor_employee" value="" ><option  value=""><?php echo $this->lang->line('Select_Visitor_Employee');?> </option><?php  foreach ($employees as $employee) { ?><option value="<?php echo $employee["id"]?>"  <?php if($value ==  $employee["id"]) echo "selected=selected";?> ><?php echo $employee["name"]?></option><?php } ?></select>
			<div class="form-error"><?= form_error('visitor_employee'); ?></div>
		</div>
	<div class="form-group col-sm-4">
		    	<?php if($data)
			{
	...
	<div class="form-group col-sm-4">
			...
			{
				$value=set_value('meeting_type');
			} ?>
			
			<label for="meeting_type" class=" form-control-label">Meeting type<?php echo $this->lang->line('meeting_type');?></label>
			<?= form_input(['name'=>'meeting_type','placeholder'=>'','class'=>'form-control','value'=>$value]);?>
		</div>
		<!--<div class="form-group col-sm-6">
			<label for="export_entry_no" class=" form-control-label">Entry No.</label>
			<?php if($data)
			{
				$value=$data->entry_no;
			}
		...
		<div class="form-group col-sm-6">
			<label for="export_entry_no" class=" form-control-label">Outgoing Entry</label>
			<?php if($data)
			{
		...
		<div class="form-group col-sm-6">
			<label for="import_entry_no" class=" form-control-label">Incoming Entry</label>
			<?php if($data)
			{
		...
		<div class="form-group col-sm-12">
			<label for="note" class=" form-control-label"><?php echo $this->lang->line('Requirement');?>*</label>
			<?php if($data)
			{
				$value=$data->note;
			}
                            ...
                            <div class="col-md-6">
								...
								<h3><?php echo $this->lang->line('Audio_Record_List');?></h3>
                                    <div class="m-widget4">
											 ...
											 <?php
												$audio = $this->db->select('audio,id,user_id,create_date')->where('audioid',$session_number)->where('type','visiting')->get('uploads')->result_array();
                                ...
                                <div class="form-field-container">
                                    ...
                                    <div class="m-widget4">
											...
											<?php
										$file = $this->db->select('*')->where("(temp_app_id = '$temp_app_id' AND cat_id = 8 AND type='visiting')", NULL, FALSE)->get('document')->result_array();
...
<?php 
                        ...
                        <a href="<?php echo base_url("admin/mission_visiting/add_mission/$case_id/".$data->id); ?>" class="btn btn-primary btn-lg">Add Session Sub Mission</a>
                        <?php  } else { ?>
...
<span class="audioocaseid" id="<?php echo $case_id; ?>"></span>
 <span class="sessionid" id="<?php echo $this->session->userdata('admin_id'); ?>"></span> <span class="type" id="visiting"></span>
 <span class="audioid" id="<?php echo $session_number; ?>"></span>
...
statusBarWidth:300,
formData: {"fid":<?php echo $case_id; ?>,"cat_id":8, "type":"visiting","temp_app_id":<?php echo $temp_app_id; ?>},
dragdropWidth:400,
...
method:'post',
url:url,
data: {'note':note,'case_id':case_id,'type':'visiting','app_id':app_id},
