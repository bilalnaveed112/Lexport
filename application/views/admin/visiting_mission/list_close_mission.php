...
<th><?php echo $this->lang->line('Subject');?> <br><i class="fa fa-arrow-up" style="font-size: 10px;font-weight: normal"></i><i class="fa fa-arrow-down" style="font-size: 10px;font-weight: normal"></i></th>	<th><?php echo $this->lang->line('Date');?><br><i class="fa fa-arrow-up" style="font-size: 10px;font-weight: normal"></i><i class="fa fa-arrow-down" style="font-size: 10px;font-weight: normal"></i></th>
		...
		?>
				...
				<?php if($appoinment['report_file']) { ?>
					...
					<?php } ?> 
					<?php $p_data=$this->db->select('*')->where('mission_id',$appoinment['id'])->where('mission_type','visiting_mission')->get('mission_rating')->row(); $id =$appoinment["id"];
					if(!isset($p_data)){ ?> 
						<a href='<?= base_url("admin/admin/review_mission/$id/visiting_mission") ?>'  title="<?php echo $this->lang->line('Add_Review');?>" class="fa fa-star" target=""></a>
						<?php
					}
