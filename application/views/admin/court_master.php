                    ...
                    <div class="m-portlet__head-caption">
                        <div class="m-portlet__head-title">
                            <h3 class="m-portlet__head-text">
                                <?php echo $this->lang->line('Court_Master');?>
                            </h3>
                        </div>
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M7.70679 13.7006C7.51926 13.888 7.26495 13.9934 6.99979 13.9934C6.73462 13.9934 6.48031 13.888 6.29279 13.7006L0.292786 7.70057C0.105315 7.51304 0 7.25874 0 6.99357C0 6.72841 0.105315 6.4741 0.292786 6.28657L6.29279 0.286571C6.48139 0.104413 6.73399 0.00361876 6.99619 0.00589718C7.25838 0.0081756 7.5092 0.113345 7.6946 0.298753C7.88001 0.484161 7.98518 0.734973 7.98746 0.99717C7.98974 1.25937 7.88894 1.51197 7.70679 1.70057L3.41379 5.99357H14.9998C15.265 5.99357 15.5194 6.09893 15.7069 6.28646C15.8944 6.474 15.9998 6.72835 15.9998 6.99357C15.9998 7.25879 15.8944 7.51314 15.7069 7.70068C15.5194 7.88821 15.265 7.99357 14.9998 7.99357H3.41379L7.70679 12.2866C7.89426 12.4741 7.99957 12.7284 7.99957 12.9936C7.99957 13.2587 7.89426 13.513 7.70679 13.7006Z" fill="#333333"/>
 ...
 Back</a>
                            <ul>
								...
								<li class="active"> <a href="http://lexport.demosbywpsqr.com/admin/admin/court_master">Court</a> </li>
	...
	<h3><?php echo $this->lang->line('COURT_LIST');?></h3>
	...
	<div class="table-responsive transparent-table">
			...
			<thead>
			  <tr class="netTr">
				...
				<th><?php echo $this->lang->line('Court_No');?></th>
			...
			<tbody>
			 <?php $ct = 0; foreach ($court_master as $ser) { $ct++;?>
			 	<tr class="hide<?= $ser->id ?>">
			 		<td><?= $ct; ?></td>
			 		<td><?= $ser->name; ?></td>
			 		<td><?= $ser->number; ?></td>
			 		<td><?= $ser->address; ?></td>
			 		<td><?= $ser->judicial_circuit; ?></td>
			 		<td class="action">
			 			<a href=<?= base_url("admin/admin/find_court_master/{$ser->id}") ?> class="fa fa-edit" title="<?php echo $this->lang->line('Edit_Court');?>"></a>&nbsp;
			 			<a href="javascript:;" class="fa fa-trash deleteservices" id='<?= $ser->id; ?>' title="<?php echo $this->lang->line('Delete_Court');?>"></td>
			 		</tr>
			<?php } ?> 
			</tbody>
		  </table>
     ...
     <div class="col-lg-12">
						<div class="form-field-container">
						<?php echo form_open('admin/admin/add_court_master',['id'=>'branch']); 
			if($data)
			{
		...
		?>
		<h3>	<?php  if($data){ ?>
				<?php echo $this->lang->line('Edit_Court');?>
	<?php	} else { ?>
	...
	<div class="form-group col-sm-12">
			<label for="name" class=" form-control-label"><?php echo $this->lang->line('Court_Name');?></label>
	<?php if($data)
			{
				$value=$data->name;
			}
			...
			echo form_input(['name'=>'name','class'=>'form-control','id'=>'name','value'=>$value]); ?>	
			<?= form_error('name'); ?>		
		</div>
		
		<div class="form-group col-sm-12">
			<label for="name" class=" form-control-label"><?php echo $this->lang->line('Court_Number');?></label>
	<?php if($data)
			{
				$value=$data->number;
			}
			...
			echo form_input(['name'=>'number','class'=>'form-control','id'=>'number','value'=>$value]); ?>	
			<?= form_error('number'); ?>		
		</div>
		
		<div class="form-group col-sm-12">
			<label for="name" class=" form-control-label"><?php echo $this->lang->line('Court_Address');?></label>
	<?php if($data)
			{
				$value=$data->address;
			}
			...
			echo form_textarea(['name'=>'address','class'=>'form-control','id'=>'address','rows'=>'3','value'=>$value]); ?>	
			<?= form_error('address'); ?>		
		</div>
		
		<div class="form-group col-sm-12">
			<label for="name" class=" form-control-label"><?php echo $this->lang->line('Judicial_Circuit');?></label>
	<?php if($data)
			{
				$value=$data->judicial_circuit;
			}
			else
			{
				$value=set_value('judicial_circuit');
			}
			echo form_input(['name'=>'judicial_circuit','class'=>'form-control','id'=>'judicial_circuit','value'=>$value]); ?>	
			<?= form_error('judicial_circuit'); ?>		
		</div>
