<!-- start add modal -->
<div class="modal fade lp-theme-modal" id="add-consultation-modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
...
<?php 
    ...
    <!--end::Portlet-->
                ...
                <!-- start edit modal -->
                <div class="modal fade lp-theme-modal" id="edit-consultation-modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                    ...
                    <div class="form-group m-form__group row">
                        ...
                        <div class="form-group col-sm-6">
                            <label for="consultation_code" class=" form-control-label"><?php echo $this->lang->line('E_Service_Number');?></label>
                            <?= form_input(['id'=>'case_id','placeholder'=>'','class'=>'form-control case_id','value'=>$current_data->case_number,'readonly'=>'readonly']);?>
                        </div>
                        <div class="form-group col-sm-6">
                            <label for="session_date" class=" form-control-label"><?php echo $this->lang->line('Consultation_Date');?>*</label>
                            <?php if($current_data)
                            ...
                            <!-- <div class="input-group date"> -->
                                <?= form_input(['name'=>'session_date','id'=>'session_date','class'=>'form-control appdate session_date','value'=>$value]);?>
                            <div class="input-group-append">
                            <span class="input-group-text"><i class="fa fa-calendar-check-o"></i></span>
                            </div>
                            <!-- </div> -->
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <?php if($current_data)
                            {
                                $value=$current_data->session_time;
                            }
                            else
                            {
                                $value=set_value('session_time');
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <?php if($data)
                            {
                                $value=$current_data->session_end_date;
                                $parts = explode('/', $value);
                            if($this->session->userdata('admin_site_lang')=="arabic" OR $this->session->userdata('admin_site_lang')=="") { 
                                $value = Greg2Hijri($parts[0], $parts[1], $parts[2],true);
                            }
    ...
    <!--end::Portlet-->
                ...
                <!-- start view modal -->
                <div class="modal fade lp-theme-modal" id="view-consultation-modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
  ...
  {
    $(document).on( 'click', '.editing-consultation-modal', function(){
        var id = $(this).attr('data-id');
        var data = $(this).find('.appointment_data').data('array');
        var service_no = $(this).find('.appointment_data_service_number').data('array');
        $('#edit-consultation-modal .data_id').val(data.id);
        $('#edit-consultation-modal .data_case_id').val(data.case_id);
        $('#edit-consultation-modal .data_client_file_number').val(data.client_file_number);
        $('#edit-consultation-modal #case_id').val(data.case_number);

        $('#edit-consultation-modal .client_name').val(data.client_name);
        $('#edit-consultation-modal .session_number').val(data.session_number);
        $('#edit-consultation-modal .session_date').val(data.session_date);
        $('#edit-consultation-modal .session_time').val(data.session_time);
        $('#edit-consultation-modal .session_end_date').val(data.session_end_date);
        $('#edit-consultation-modal .session_end_time').val(data.session_end_time);
        $('#edit-consultation-modal .session_code').val(data.session_code);
        $('#edit-consultation-modal .subject').val(data.subject);
        $('#edit-consultation-modal .consultation_type').val(data.consultation_type);
        $('#edit-consultation-modal #comment').val(data.note);
        $('#edit-consultation-modal .decision').val(data.decision);
        $('#edit-consultation-modal .report').val(data.report);
        var baseUrlConsult = $('#edit-consultation-modal .base_url').val();
        $('#edit-consultation-modal .sb-btn').attr('href', baseUrlConsult + data.case_id + '/' + data.id);
    });

    $(document).on( 'click', '.viewing-consultation-modal', function(){
        var id = $(this).attr('data-id');
        var data = $('.editing-consultation-modal[data-id="' + id + '"]').find('.appointment_data').data('array');
        $('#view-consultation-modal .session_number').text(data.session_number);
        $('#view-consultation-modal .case_number').text(data.case_number);
        $('#view-consultation-modal .session_date').text(data.session_date);
        $('#view-consultation-modal .session_time').text(data.session_time);
        $('#view-consultation-modal .session_end_date').text(data.session_end_date);
        $('#view-consultation-modal .session_end_time').text(data.session_end_time);
        $('#view-consultation-modal .session_code').text(data.session_code);
        $('#view-consultation-modal .consultation_type').text(data.consultation_type);
        $('#view-consultation-modal .responsible_employee').text(data.responsible_employee);
        $('#view-consultation-modal .follow_up_employee').text(data.follow_up_employee);
        $('#view-consultation-modal .subject').text(data.subject);
        $('#view-consultation-modal #comment').text(data.note);
        $('#view-consultation-modal .report').text(data.report);       
    });
    $(document).on("click", ".consultation .delete_appoinment", function() {
