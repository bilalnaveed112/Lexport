 ...
 Back</a>
		...
		?>
				...
				<?php if($appoinment['report_file']) {?>
					...
					<?php } ?>
					<?php $p_data=$this->db->select('*')->where('mission_id',$appoinment['id'])->where('mission_type','general_mission')->get('mission_rating')->row(); $id =$appoinment["id"];
					if(!isset($p_data)){ ?> 
						<a href='<?= base_url("admin/admin/review_mission/$id/general_mission") ?>'  title="<?php echo $this->lang->line('Add_Review');?>" class="fa fa-star" target=""></a>
						<?php
					}
					else if($p_data->follow_rating == 0 || $p_data->responsible_rating == 0  || $p_data->master_rating== 0 ){
					?>
						<a href='<?= base_url("admin/admin/review_mission/$id/general_mission") ?>'  title="<?php echo $this->lang->line('Add_Review');?>" class="btn btn-info" target=""><i class="fa fa-star"></i> <?php echo $this->lang->line('Add_Review');?></a>
						<?php
					}
					?>
                </td>
