...
<th><?php echo $this->lang->line('Subject');?> <br><i class="fa fa-arrow-up" style="font-size: 10px;font-weight: normal"></i><i class="fa fa-arrow-down" style="font-size: 10px;font-weight: normal"></i></th><th><?php echo $this->lang->line('Following_Employee');?><br><i class="fa fa-arrow-up" style="font-size: 10px;font-weight: normal"></i><i class="fa fa-arrow-down" style="font-size: 10px;font-weight: normal"></i></th>
 ...
 <td><?= $appoinment['subject'] ?></td>
				 <td>
				<?php  if($appoinment['follow_up_employee'] != ''){ echo getEmployeeName($appoinment['follow_up_employee']); } ?>
				</td>
				...
				<td><span class='countdown' style=" color: #0e8a00; font-weight: bold; " value='<?php echo mission_due_time($appoinment['session_end_date'],$appoinment['session_end_time']);?>' data-countdown='<?php echo mission_due_time($appoinment['session_end_date'],$appoinment['session_end_time']);?>'></span></td>
		          <td class="action">
                    ...
                    <a href=<?= base_url("admin/$control/find_mission/{$appoinment['id']}") ?> class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill editadmin" id=<?= $appoinment['id'] ?>><i class="fa fa-pencil-square-o " title="<?php echo $this->lang->line('Edit');?>"></i></a>
					<?php  } ?>
				
				<?php if($this->session->userdata('role_id') == 1  ){?>
                    <a href="javascript:;" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill delete_appoinment" id=<?= $appoinment['id'] ?> title="<?php echo $this->lang->line('Delete');?>"><i class="fa fa-trash"></i></a>
					 <?php  } ?>
					 <?php  } ?>
					 <?php if(isset($datas[4][3]) && $datas[4][3] == 1) { ?>
					<a href=<?= base_url("admin/$control/view_mission/{$appoinment['id']}") ?>  title="<?php echo $this->lang->line('View');?>" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill " ><i class="fa fa-eye"></i></a>
					 <?php  } ?>
					 <?php if($appoinment['is_close'] == 1 ){?>
		
					 	<a href="#" title="<?php echo $this->lang->line('Close');?>" class=" btn btn-danger btn-sm"><?php echo $this->lang->line('Close');?></a>
					  <?php  } ?>
					<?php if($appoinment['is_convert'] == 1 ){ ?>
					<a href="javascript:;" class="btn btn-info btn-sm" title="<?php echo $this->lang->line('Convert_Case');?>"><?php echo $this->lang->line('Converted');?></a>
					<?php } ?>
					</span>
                </td>
              </tr>
