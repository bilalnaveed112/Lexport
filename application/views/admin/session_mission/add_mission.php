...
<?php echo $js["judicial_circuit"]?></option><?php } ?></select>
		...
		<script>
			...
			<audio controls id="audio" name="audio"></audio>
		<div class="btn-group">
		...
		<div data-type="wav">
			<p class="mb-4" ><?php echo $this->lang->line('WAV_Controls');?>:</p>
                            ...
                            <div class="col-md-6">
								...
								<h3><?php echo $this->lang->line('Audio_Record_List');?></h3>
                                    <div class="m-widget4">
											...
											<?php
		$audio = $this->db->select('audio,id,user_id,create_date')->where('audioid',$session_number)->where('type','session')->get('uploads')->result_array();
		foreach ($audio as $audio) {  $timestamp = strtotime($audio['create_date']); $date  =   date("d M Y G:i",$timestamp); ?>
		   <div class="m-widget4__item">
						   ...
						   <div class="form-field-container">
                                    ...
                                    <div class="m-widget4">
										<?php
										$file = $this->db->select('*')->where("(temp_app_id = '$temp_app_id' AND cat_id = 8 AND type='session')", NULL, FALSE)->get('document')->result_array(); 
						...
						<div class="row">
							...
							<div class="col-12">
								...
								<a href="<?php echo base_url("admin/mission_session/add_mission/$case_id/".$data->id); ?>" class="btn btn-primary btn-lg"><?php echo $this->lang->line('Add_Session_Sub_Mission')?></a>
								<?php  } else { ?>
...
?>
 ...
 <span class="sessionid" id="<?php echo $this->session->userdata('admin_id'); ?>"></span> <span class="type" id="session"></span>
...
<script>
jQuery('#close_app_new').on('click', function (e) {
e.preventDefault();
  bootbox.confirm("Current mission will close if you create new mission", function(result){
      if(result)
      {
           jQuery('<input />').attr('type', 'hidden')
          .attr('name', "close_app_new")
          .attr('value', "close_app_new")
          .appendTo('#customer');
        jQuery('#customer').submit();
        return true;
      }
      else
      {
...
$(".bootbox-body").on("change", ".rating input", function() {  
  var $radio = $(this);
...
formData: {"fid":<?php echo $case_id; ?>,"cat_id":8, "type":"session","temp_app_id":<?php echo $temp_app_id; ?>},
dragdropWidth:400,
...
method:'post',
url:url,
data: {'note':note,'case_id':case_id,'type':'session','app_id':app_id},
...
$(document).ready(function(){
	...
	$('#responsible_employee').change(function() {
		// Get the selected value from the Responsible Employee dropdown
		var responsibleValue = $(this).val();
		
		// Update the Follow-up Employee dropdown to prevent selecting the same value
		$('#employee_id option').each(function() {
			if ($(this).val() == responsibleValue) {
				$(this).prop('disabled', true);  // Disable the option if it's the same
			} else {
				$(this).prop('disabled', false); // Enable the option if it's different
			}
		});
	});

	// Listen for changes in the Follow-up Employee dropdown
	$('#employee_id').change(function() {
		// Get the selected value from the Follow-up Employee dropdown
		var followUpValue = $(this).val();
		
		// Update the Responsible Employee dropdown to prevent selecting the same value
		$('#responsible_employee option').each(function() {
			if ($(this).val() == followUpValue) {
				$(this).prop('disabled', true);  // Disable the option if it's the same
			} else {
				$(this).prop('disabled', false); // Enable the option if it's different
			}
