...
class dashboard_mod extends CI_Model {
	...
	public function user_activity($id) {
		...
		$Sql = "SELECT * FROM action_log WHERE (customer_id=$id OR user_id = $id) AND create_date >=  (CURDATE() + INTERVAL 1 DAY) ORDER BY id DESC";
	 
		$user_activity  = $this->db->query($Sql)->result_array();
		
		return $user_activity;
		
	}
	public function e_service_data() {
		$mission['data']['padding'] = $this->db->select('*')->where(['is_reject'=>0])->order_by("id", "desc")->get('case_temp')->num_rows();
		
		$mission['data']['reject'] =  $this->db->select(['case_id','client_name','customers_id','is_reject','service_types','contract_number','add_edit','reject_note'])->where(['is_reject'=>1])->order_by("id", "desc")->get('case_temp')->num_rows();
		...
		$mission['data']['total']=$this->db->get()->num_rows();
		return $mission;
	}
	public function mission_data() {
		
		$this->db->select("*")->where('is_close',1);
		$mission['data']['mission_close']['general'] = $this->db->get('general_mission')-> num_rows();
		$this->db->select("*")->where('is_close',1);
		$mission['data']['mission_close']['session'] = $this->db->get('session_mission')-> num_rows();
		$this->db->select("*")->where('is_close',1);
		$mission['data']['mission_close']['visiting'] = $this->db->get('visiting_mission')-> num_rows();
		$this->db->select("*")->where('is_close',1);
		$mission['data']['mission_close']['writing'] = $this->db->get('writing_misssion')-> num_rows();
		$this->db->select("*")->where('is_close',1);
		$mission['data']['mission_close']['consultation'] = $this->db->get('consultation_mission')-> num_rows();
	
		$this->db->select("*");
		$mission['data']['mission_total']['general'] = $this->db->get('general_mission')-> num_rows();
		$this->db->select("*");
		$mission['data']['mission_total']['session'] = $this->db->get('session_mission')-> num_rows();
		$this->db->select("*");
		$mission['data']['mission_total']['visiting'] = $this->db->get('visiting_mission')-> num_rows();
		$this->db->select("*");
		$mission['data']['mission_total']['writing'] = $this->db->get('writing_misssion')-> num_rows();
		$this->db->select("*");
		$mission['data']['mission_total']['consultation'] = $this->db->get('consultation_mission')-> num_rows();
		return $mission;
	}
	public function invoice_data()  {
			...
			->where('invoice_details.payment_status','paid');
			$invoice['paid'] = $this->db->get('invoice')-> num_rows();
			
			$this->db->select("invoice.*,invoice_details.*,c_case.id,c_case.branch,") 
			->join('c_case', "c_case.id = invoice.case_id")
			->join('invoice_details', "invoice_details.invoice_id = invoice.id", 'left')
			->where('invoice_details.payment_status','unpaid');
			$invoice['unpaid'] = $this->db->get('invoice')-> num_rows();
			
			$this->db->select("invoice.*,invoice_details.*,c_case.id,c_case.branch,") 
			->join('c_case', "c_case.id = invoice.case_id")
			->join('invoice_details', "invoice_details.invoice_id = invoice.id", 'left')
			->where('invoice_details.payment_status','hold');
			$invoice['hold'] = $this->db->get('invoice')-> num_rows();
			
			$this->db->select("invoice.*,invoice_details.*,c_case.id,c_case.branch,") 
			->join('c_case', "c_case.id = invoice.case_id")
			->join('invoice_details', "invoice_details.invoice_id = invoice.id", 'left')
			->where('invoice_details.payment_status','process');
			
			$invoice['process'] = $this->db->get('invoice')->num_rows();
			$invoice_data['data']['count'] = $invoice;

			$timeSQL = '';
			$timeSQL .= ' sum(case when payment_status = "paid" then total else 0 end) AS paid_amount,';
			$timeSQL .= ' sum(case when payment_status = "unpaid" then total else 0 end) AS unpaid_amount,';
			$timeSQL .= ' sum(case when payment_status = "hold" then total else 0 end) AS hold_amount,';
			$timeSQL .= ' sum(case when payment_status = "process" then total else 0 end) AS process_amount';
			
			$Sql = "SELECT ".$timeSQL." FROM invoice_details";
			$invoice_amount  = $this->db->query($Sql)->row_array();	
			$invoice_data['data']['amount'] =$invoice_amount;
			...
			$expense = $this->db->get('expense')->result_array();
			 ...
			 $invoice_data['data']['expense'] = $expense;
			 return $invoice_data;
	}
	public function active_user()  {
	$aid = $this->session->userdata('activity_id');
			 
		return	$this->db->select("*")
			//->where("users.role_id",3)
			->where("session_id !=",0)
			->or_where("session_id !=",1)
			->where("last_login_time >", 'CURDATE()', FALSE)
	...
	$aid = 	 $this->session->userdata('activity_id');
		...
		return	$this->db->select("*")
			...
			//->where("users.role_id",3)
			->where("users.session_id !=",0)->or_where("session_id !=",1)->where("last_login_time >", 'CURDATE()', FALSE)
			->order_by("last_login_time",'desc')
			->get('users')
			->num_rows();
	}	
	public function count_today_active_users()  {
		$date = new DateTime("now");

		$curr_date = $date->format('Y-m-d G:i:s');

		$this->db->select('*');
		$this->db->from('users'); 
	 
		$this->db->where("last_login_time >", 'CURDATE()', FALSE);
		$query = $this->db->get();
		return $query->num_rows();
			 
	}
	public function today_active_users()  {
		$date = new DateTime("now");
	...
	{
		if($this->session->userdata('role_id') == 1){
		$q = $this->db->select(['client_name', 'type_of_customer', 'client_file_number','id'])->limit(5)->get('customers');
		} else {
...
/* 
		...
		$all_report[0][8] = 'Today'; 
		$all_report[1][8] = 'Tomorrow'; 
		$all_report[2][8] = 'Next Week';  
		$all_report[3][8] = 'Next Month'; 
	...
	// general  		
		...
		$all_report[0][9] = 'Running'; 
		$all_report[1][9] = 'Close'; 
