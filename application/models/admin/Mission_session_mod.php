<?php
class Mission_session_mod extends CI_Model
{
	...
	public function list_mission()
	{
		...
		$role_id = $this->session->userdata('role_id');
			...
			if($this->uri->segment(3) == "list_responsible"){
				...
				$where  ="{$table}.is_close = 0 AND {$table}.on_review = 0  AND {$table}.on_review = 0";
			}
			...
			return  $this->db->select("$table.*,c_case.id as case_id,c_case.client_name,c_case.case_number,c_case.customers_id,c_case.service_types,c_case.opponent_full_name,c_case.case_title,c_case.court_name,c_case.judge_name")
 	        ->join('c_case', "c_case.id = {$table}.case_id", 'left')
		...
		else
		{
			...
			// ASSIGN //
			$where  ="assign_mission.following_employee_id = $user_id AND {$table}.is_close = 0  AND {$table}.on_review = 0  AND assign_mission.type='session_mission'";
			...
			return  $this->db->select("{$table}.*,c_case.id as case_id,c_case.client_name,c_case.customers_id,c_case.case_number,c_case.service_types,c_case.opponent_full_name,c_case.case_title,c_case.court_name,c_case.judge_name")
 	        ->join('c_case', "c_case.id = {$table}.case_id", 'left')
			->join('assign_mission', "assign_mission.following_employee_id = $user_id",'left')
			->where("($where)", NULL, FALSE)
			->where("{$table}.sub_mission_id",$sub_mission_id)
			->where("{$table}.status",0)->order_by("{$table}.id", "desc")
			//->where('mission.is_close',0)
