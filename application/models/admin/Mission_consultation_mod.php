<?php
class Mission_consultation_mod extends CI_Model
{
	...
	public function app_note_data($id){
		return $note_data=$this->db->select('*')->where('app_id',$id)->where('type','consultati')->get('case_note')->result_array();
	}
	public function task_type(){
		return $note_data=$this->db->select('*')->order_by("id", "desc")->get('task_type')->result_array();
	}
	...
	public function list_mission()
	{
		...
		if($this->uri->segment(3) == "list_responsible"){
				...
				// ASSIGN //
			$where  ="assign_mission.following_employee_id = $user_id AND {$table}.is_close = 0  AND {$table}.on_review = 0 AND assign_mission.type='consultation_mission'";
