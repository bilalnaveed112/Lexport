<?php
class Mission_visiting_mod extends CI_Model
{
	...
	public function app_note_data($id){
		return $note_data=$this->db->select('*')->where('app_id',$id)->where('type','visiting')->get('case_note')->result_array();
	}
	...
	public function list_mission()
	{
		...
		$role_id = $this->session->userdata('role_id');
			...
			if($this->uri->segment(3) == "list_responsible"){
				...
				// ASSIGN //
			$where  ="assign_mission.following_employee_id = $user_id AND {$table}.is_close = 0  AND {$table}.on_review = 0 AND assign_mission.type='visiting_mission'";
		...
		if($this->uri->segment(3) == "list_responsible"){
				...
				$where  ="{$table}.follow_up_employee = $user_id AND {$table}.is_close = 0  AND {$table}.on_review = 0";
			} 
			else if($this->uri->segment(3) == "list_close_mission"){
