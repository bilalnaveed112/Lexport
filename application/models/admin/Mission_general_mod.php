<?php
class Mission_general_mod extends CI_Model
{
	...
	public function app_note_data($id){
		return $note_data=$this->db->select('*')->where('app_id',$id)->where('type','general')->get('case_note')->result_array();
	}
	...
	public function list_mission()
	{
		...
		$role_id = $this->session->userdata('role_id');
			...
			else if($this->uri->segment(3) == "list_follow_up AND {$table}.on_review = 0"){
		...
		else
		{
				...
				// ASSIGN //
			$where  ="assign_mission.following_employee_id = $user_id AND {$table}.is_close = 0  AND {$table}.on_review = 0  AND assign_mission.type='general_mission'";
