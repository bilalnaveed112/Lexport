<?php
class Mission_general extends MY_Controller {
	private $table_name= "general_mission";
	...
	public function add_mission($id){
		...
		$note_data=$this->mission_general_mod->app_note_data($id);
		$task_type=$this->mission_general_mod->task_type();
		$this->session->unset_userdata('temp_app_id');
		return $this->load->view('admin/general_mission/add_mission',['data'=>'','case_data'=>$case_data,'note_data'=>$note_data,'employees'=>$employees,'app_data'=>'','task_type'=>$task_type]);
	}
	...
	// Submission
		...
		->where("{$table}.on_review",1)->where("{$table}.sub_mission_id",$sub_mission_id)
	...
	//	->where("{$table}.user_id", $user_id )
		...
		$this->load->view('admin/general_mission/list_review', ['data' => $data,'list_customer'=>$list_customer]);
}
	public function approve_mission($id) {
		$data = $this->mission_general_mod->find_mission($id);
        $closepost['close_date'] = date("Y-m-d G:i:s");
    	$this->db->where('id',$id)->update($this->table_name,$closepost);
		$case_data=$this->mission_general_mod->app_case_data($data->case_id);
		$session_date=$this->db->select('case_id,session_date')->where('case_id',$data->case_id)->get($this->table_name)->result_array();
		$filename = "general_mission_report_".time().".pdf";
		ob_start();
        $pdfFilePath = 'uploads/case_file/'."$filename";
        $this->load->library('m_pdf');
	  	$html=$this->load->view('admin/general_mission/report', ['data' => $data,'case_data'=>$case_data,'session_date'=>$session_date], true);  
		ERROR_REPORTING(0);
...
$this->m_pdf->pdf->Output($pdfFilePath,"F");  
    ...
    $user_email = implode(',',$user_email);
		...
		$this->email->subject($this->lang->line('General_mission_has_been_close'));
		...
		if($this->session->userdata('admin_site_lang')=="arabic" OR $this->session->userdata('admin_site_lang')=="") { 
	        $msg ='شريكنا العزيز: لقد أنجزت مهمة الخدمة رقم: '. getCaseNumber($noti['case_id']).' ، ويمكنكم الاطِّلاع على التفاصيل عبر إحدى منصاتنا الإلكترونية. "نسعد بخدمتكم، وممتنون لثقتكم"';
		} else {
            $msg ='Dear Partner: Your service task No. '. getCaseNumber($noti['case_id']).' was successfully accomplished. For details, please visit any of “Nassr” platforms. "We are happy to serve you and we are grateful for your trust"';
    	}
		sendSMSProcess($phone,$msg);
		$json_data['case_id'] =$noti['case_id'];
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,$case_data->customers_id,"general_mission","approve");
		return redirect("admin/admin/review_mission/$id/general_mission",'refresh');
	}
	
	public function list_mission(){
		$data = $this->mission_general_mod->list_mission();
		$json_data['page']='full list';
		insertActionLog($json_data,0,"general_mission","visit");
		return $this->load->view('admin/general_mission/list_mission', ['data' => $data]);
	...
	public function list_responsible(){ 
		$data = $this->mission_general_mod->list_mission();
		$employees=$this->case_mod->get_employee();
		$json_data['page']='responsible list';
		insertActionLog($json_data,0,"general_mission","visit");
		return $this->load->view('admin/general_mission/list_responsible', ['data' => $data,'employees' => $employees,]);
	}
	...
	public function list_close_mission(){
		...
		insertActionLog($json_data,0,"general_mission","visit");
		return $this->load->view('admin/general_mission/list_close_mission', ['data' => $data]);
	}
	public function list_pending_misssion(){
		$data = $this->mission_general_mod->list_pending_mission();
		$json_data['page']='pending list';
		insertActionLog($json_data,0,"general_mission","visit");
		$this->load->view('admin/general_mission/list_pending_misssion', ['data' => $data]);
	}
	public function list_follow_up(){
		$data = $this->mission_general_mod->list_mission();
		$json_data['page']='follow list';
		insertActionLog($json_data,0,"general_mission","visit");
		return $this->load->view('admin/general_mission/list_follow_up', ['data' => $data]);
	}
	...
	$post['on_review'] = 0;
        ...
        $case_data=$this->mission_general_mod->app_case_data($data->case_id);
				...
				$this->email->from($from_email, constant("SENDER_NAME"));
				$this->email->to($to_email);
				$this->email->subject($this->lang->line('General_mission_added'));
				$body = $this->load->view('admin/add-case-email.php', $new, true);
				$this->email->message($body);
				$this->email->send();
    	    	...
    	    	if($this->session->userdata('admin_site_lang')=="arabic" OR $this->session->userdata('admin_site_lang')=="") { 
                    $msg ="شريكنا العزيز: لقد أنشئت  مهمة للخدمة رقم ". getCaseNumber($noti['case_id'])." وسنوافيكم دوماً بالمستجدات من خلال زيارتكم لإحدى منصاتنا الإلكترونية.";
        		} else {
        		    $msg ="Dear Partner: Task service No ". getCaseNumber($noti['case_id'])."  has been created and we will always update you. For details, please visit any of “Nassr” platforms";
     			}
            sendSMSProcess($phone,$msg);
		
		$json_data['page']='pending list';
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"general_mission","pending-approve");
		return redirect('admin/mission_general/list_pending_misssion');
	}
	public function reject_pending_mission($id) {
		...
		$this->db->where('id',$id)->update($this->table_name,$post);
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"general_mission","reject");
		return redirect('admin/mission_general/list_pending_misssion');
	}
	...
	public function find_mission($id) 
	{
		$employees=$this->case_mod->get_employee();
		$data = $this->mission_general_mod->find_mission($id);
		$app_data = $this->mission_general_mod->app_data($id);
		$case_data=$this->mission_general_mod->app_case_data($data->case_id);
		$note_data=$this->mission_general_mod->app_note_data($id);
		$task_type=$this->mission_general_mod->task_type();
		$json_data['misssion_id'] =$id;
		$json_data['page'] ='edit';
		insertActionLog($json_data,0,"general_mission","visit");
		return $this->load->view('admin/general_mission/add_mission', ['data' => $data,'employees'=>$employees,'case_data'=>$case_data,'note_data'=>$note_data,'app_data'=>$app_data,'task_type'=>$task_type]);
	}
 ...
 //$read['is_read'] = 1;
	...
	//	$data = $this->mission_general_mod->find_mission($id);
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"general_mission","view");
		return $this->load->view('admin/general_mission/view_mission', ['data' => $data, ]);
	}
	public function print_pdf($id)
	{
		$data = $this->mission_general_mod->find_mission($id);
		$pdfFilePath = "mission_general".time().".pdf";
        $this->load->library('m_pdf'); 
	...
	public function delete_mission() 
	{
		...
		$this->mission_general_mod->delete_mission($id);
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"general_mission","delete");
...
*/
		...
		insertActionLog($json_data,0,"general_mission","update");
	//Report
	if($close_app == 1){
...
$this->m_pdf->pdf->Output($pdfFilePath,"F");  
		...
		$json_data['misssion_id'] =$id;
		$json_data['case_id'] =$case_data->id;
		insertActionLog($json_data,$case_data->customers_id,"general_mission","close");
		$users=$this->db->select('*')->where('id',$noti['customer_id'])->get('users')->row_array();
    ...
    $user_email = implode(',',$user_email);
		...
		//Submission
		$eid = $noti['case_id'];
        return redirect("admin/admin/review_mission/$id/general_mission/?is_new=$close_app_new&eid=$eid",'refresh');
        //Submission
 
	}
			return redirect('admin/mission_general/list_mission');
				...
				$post_data['type']='general_mission';
               ...
               if(isset($case_data->customers_id))
				{
					...
					$json_data['case_id'] = $case_data->id;
					insertActionLog($json_data,$case_data->customers_id,"general_mission","add");
					//email
				$users=$this->db->select('*')->where('id',$noti['customer_id'])->get('users')->row_array(); 
				$email = $users['email'];
				$name = $users['name']; 
			...
			if($id){
				$this->load->view('admin/general_mission/add_mission',['data'=>'','employees'=>$employees,'app_data'=>'','case_data'=>$case_data,'note_data'=>$note_data]);
			} else {
				$this->load->view('admin/general_mission/add_mission',['data'=>'','employees'=>$employees,'app_data'=>'','case_data'=>$case_data,'note_data'=>$note_data]);
			}
		}	
