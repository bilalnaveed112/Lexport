<?php
class Mission_consultation extends MY_Controller {
	private $table_name= "consultation_mission";
	public function __construct() {
		parent::__construct();
	$this->isAdmintLogged();    
		$this->load->model('admin/mission_consultation_mod');
	...
	public function add_mission($id){
		...
		$note_data=$this->mission_consultation_mod->app_note_data($id);
		$task_type=$this->mission_consultation_mod->task_type();
		$this->session->unset_userdata('temp_app_id');
		return $this->load->view('admin/consultation_mission/add_mission',['data'=>'','case_data'=>$case_data,'note_data'=>$note_data,'employees'=>$employees,'app_data'=>'','task_type'=>$task_type]);
	}
	...
	$this->load->view('admin/consultation_mission/list_review', ['data' => $data,'list_customer'=>$list_customer]);
	}
	public function approve_mission($id) {
		$data = $this->mission_consultation_mod->find_mission($id);
		$closepost['close_date'] = date("Y-m-d G:i:s");
		$closepost = $this->security->xss_clean($closepost);
    	$this->db->where('id',$id)->update($this->table_name,$closepost);
		$case_data=$this->mission_consultation_mod->app_case_data($data->case_id);
		$session_date=$this->db->select('case_id,session_date')->where('case_id',$data->case_id)->get($this->table_name)->result_array();
		$filename = "consultation_mission_report_".time().".pdf";
		ob_start();
        $pdfFilePath = 'uploads/case_file/'."$filename";
        $this->load->library('m_pdf');
	  	$html=$this->load->view('admin/consultation_mission/report', ['data' => $data,'case_data'=>$case_data,'session_date'=>$session_date], true);  
		ERROR_REPORTING(0);
...
$this->m_pdf->pdf->Output($pdfFilePath,"F");  
    ...
    $user_email = implode(',',$user_email);
		...
		$this->email->subject($this->lang->line('Consultation_mission_has_been_close'));
		...
		if($this->session->userdata('admin_site_lang')=="arabic" OR $this->session->userdata('admin_site_lang')=="") { 
             $msg='شريكنا العزيز: لقد أنجزت مهمةُ الاستشارة  للخدمة  رقم: '. getCaseNumber($noti['case_id']).' بنجاح، ويمكنكم الاطِّلاع على التفاصيل عبر إحدى منصاتنا الإلكترونية. "نسعد بخدمتكم، وممتنون لثقتكم"';
		} else { 
		    $msg= 'Dear Partner: Your legal consulting service task No. '. getCaseNumber($noti['case_id']).' was successfully accomplished. For details, please visit any of “Nassr” platforms. "We are happy to serve you and we are grateful for your trust"';
		}
		sendSMSProcess($phone,$msg);
		$json_data['case_id'] =$noti['case_id'];
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,$case_data->customers_id,"/consultation_mission","approve");
 
		return redirect("admin/admin/review_mission/$id/consultation_mission",'refresh');
	}
	
	public function list_mission(){
		$data = $this->mission_consultation_mod->list_mission();
				$json_data['page']='full list';
		insertActionLog($json_data,0,"consultation_mission","visit");
	return	$this->load->view('admin/consultation_mission/list_mission', ['data' => $data]);
	...
	public function list_responsible(){ 
		...
		insertActionLog($json_data,0,"consultation_mission","visit");
		return $this->load->view('admin/consultation_mission/list_responsible', ['data' => $data,'employees' => $employees,]);
	}
	...
	public function list_close_mission(){
		...
		$json_data['page']='close list';
		insertActionLog($json_data,0,"consultation_mission","visit");
		$this->load->view('admin/consultation_mission/list_close_mission', ['data' => $data]);
	}
	public function list_pending_misssion(){
		$data = $this->mission_consultation_mod->list_pending_mission();
		$json_data['page']='pending list';
		insertActionLog($json_data,0,"consultation_mission","visit");
		return $this->load->view('admin/consultation_mission/list_pending_misssion', ['data' => $data]);
	}
	public function list_follow_up(){
		$data = $this->mission_consultation_mod->list_mission();
		$json_data['page']='follow list';
		insertActionLog($json_data,0,"consultation_mission","visit");
		return $this->load->view('admin/consultation_mission/list_follow_up', ['data' => $data]);
	}
	public function approve_pending_mission($id) {
        ...
        $case_data=$this->mission_consultation_mod->app_case_data($data->case_id);
  		$noti['customer_id'] = $case_data->customers_id;
				...
				$this->email->from($from_email, constant("SENDER_NAME"));
				$this->email->to($to_email);
				$this->email->subject($this->lang->line('Consultation_mission_added'));
				$body = $this->load->view('admin/add-case-email.php', $new, true);
				$this->email->message($body);
				$this->email->send();
                ...
                if($this->session->userdata('admin_site_lang')=="arabic" OR $this->session->userdata('admin_site_lang')=="") { 
                    $msg ='شريكنا العزيز: لقد أنشئت مهمة استشارة للخدمة رقم '. getCaseNumber($noti['case_id']).' وسنوافيكم دومًا بالمستجدات من خلال زيارتكم لإحدى منصاتنا الإلكترونية.';
                } else { 
                    $msg='Dear Partner: A legal consulting task has been created for service number '. getCaseNumber($noti['case_id']).' and we will always keep you updated . For more details, kindly visit any of “Nassr” platforms.';
                }
                sendSMSProcess($phone,$msg);
 
		$json_data['page']='pending list';
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"consultation_mission","pending-approve");
		return redirect('admin/mission_consultation/list_pending_misssion');
	}
	public function reject_pending_mission($id) {
		...
		$this->db->where('id',$id)->update($this->table_name,$post);
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"consultation_mission","reject");
		return redirect('admin/mission_consultation/list_pending_misssion');
	}
	...
	public function find_mission($id) 
	{
		$employees=$this->case_mod->get_employee();
		$data = $this->mission_consultation_mod->find_mission($id);
		$app_data = $this->mission_consultation_mod->app_data($id);
		$case_data=$this->mission_consultation_mod->app_case_data($data->case_id);
		$note_data=$this->mission_consultation_mod->app_note_data($id);
		$task_type=$this->mission_consultation_mod->task_type();
				$json_data['misssion_id'] =$id;
		$json_data['page'] ='edit';
		insertActionLog($json_data,0,"consultation_mission","visit");
		return $this->load->view('admin/consultation_mission/add_mission', ['data' => $data,'employees'=>$employees,'case_data'=>$case_data,'note_data'=>$note_data,'app_data'=>$app_data,'task_type'=>$task_type]);
	}
    ...
    $r = $_SERVER['HTTP_REFERER']; 
		...
		$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"consultation_mission","view");
		return $this->load->view('admin/consultation_mission/view_mission', ['data' => $data, ]);
	}
	
	public function print_pdf($id)
	{
		$data = $this->mission_consultation_mod->find_mission($id);
		$pdfFilePath = "mission_consultation".time().".pdf";
        $this->load->library('m_pdf'); 
	...
	public function delete_mission() 
	{
		...
		$this->mission_consultation_mod->delete_mission($id);
				$json_data['misssion_id'] =$id;
		insertActionLog($json_data,0,"consultation_mission","delete");
 ...
 $onreview=$this->input->post('onreview');
        ...
        $post['is_read'] = isset($onreview)?'1':'0';
	...
	//	$this->db->where('id',$mid)->update('appointment_task',$tasks);
		...
		insertActionLog($json_data,0,"consultation_mission","update");
			}
	//Report
	if($close_app == 1){
		...
		$case_data=$this->mission_consultation_mod->app_case_data($data->case_id);
		$session_date=$this->db->select('case_id,session_date')->where('case_id',$data->case_id)->get($this->table_name)->result_array();
		$filename = "consultation_report_".time().".pdf";
		ob_start();
        $pdfFilePath = 'uploads/case_file/'."$filename";
        $this->load->library('m_pdf');
	  	$html=$this->load->view('admin/consultation_mission/report', ['data' => $data,'case_data'=>$case_data,'session_date'=>$session_date], true);  
...
$this->m_pdf->pdf->Output($pdfFilePath,"F");  
		...
		$noti['appointment_id'] = $id;
		$noti['user_id'] = $case_data->user_id;
		$noti['notification_type'] = 'consultation_mission';
		$noti['status_type'] = 'close'; 	
			$noti['create_date']=date("Y-m-d G:i:s");
		$this->db->insert('notification', $noti);
		$json_data['misssion_id'] =$id;
		$json_data['case_id'] =$case_data->id;
		insertActionLog($json_data,$case_data->customers_id,"consultation_mission","close"); 
		$users=$this->db->select('*')->where('id',$noti['customer_id'])->get('users')->row_array(); 
    ...
    $user_email = implode(',',$user_email);
		...
		//Submission
		$eid = $noti['case_id'];
        return redirect("admin/admin/review_mission/$id/consultation_mission/?is_new=$close_app_new&eid=$eid",'refresh');
        //Submission
 
	}
	return redirect("admin/mission_consultation/list_mission/",'refresh');
				...
				$post_data['type']='consultation_mission';
			...
			//	$this->db->insert('appointment_task',$tasks); 
				...
				if($this->session->userdata('role_id') ==1){
					...
					$json_data['case_id'] = $case_data->id;
					insertActionLog($json_data,$case_data->customers_id,"consultation_mission","add");
					//email
				$users=$this->db->select('*')->where('id',$noti['customer_id'])->get('users')->row_array(); 
				$email = $users['email'];
				$name = $users['name']; 
				...
				$this->email->from($from_email, constant("SENDER_NAME"));
				$this->email->to($to_email);
				$this->email->subject($this->lang->line('consultation_appoinment_add'));
				$body = $this->load->view('admin/add-case-email.php', $new, true);
				$this->email->message($body);
				$this->email->send();
			...
			return redirect('admin/mission_consultation/list_mission');
		} else {
			if($id){
				return $this->load->view('admin/consultation_mission/add_mission',['data'=>'','employees'=>$employees,'app_data'=>'','case_data'=>$case_data,'note_data'=>$note_data]);
			} else {
				return $this->load->view('admin/consultation_mission/add_mission',['data'=>'','employees'=>$employees,'app_data'=>'','case_data'=>$case_data,'note_data'=>$note_data]);
			}
		}	
