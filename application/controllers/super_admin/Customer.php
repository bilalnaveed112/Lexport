...
class customer extends MY_Controller {
	...
	public function view_customer($id) //je customer edit karva no 6e te find karva mate
	{
		...
		->where("session_mission.client_file_number",$client_file_number)
		->get('session_mission')
		->result_array();
		...
		->where("writing_misssion.client_file_number",$client_file_number)
		...
		->where("consultation_mission.client_file_number",$client_file_number)
		...
		->where("general_mission.client_file_number",$client_file_number)
		->get('general_mission')
		->result_array();
		...
		->where("visiting_mission.client_file_number",$client_file_number)
	 ...
	 if(!isset($project)) $project = '';
		...
		return $this->load->view('admin/view_customer', ['data' => $data,'service'=>$ser,'session'=>$session,'writings'=>$writings,'consultation'=>$consultation,'visiting'=>$visiting,'projects'=>$project,'case_list'=>$case_list,'archives'=>$archives,'user'=>$user,'get_per_procuration'=>$get_per_procuration,'get_per_tasks'=>$get_per_tasks,'general'=>$general]);
	}
	...
	public function delete_customer() //customer delete karva mate
	{
		...
		echo json_encode('customer delete successfully');
	}
